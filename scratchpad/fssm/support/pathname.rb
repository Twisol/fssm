...
module FSSM::Support
  class Pathname < String
    ...
    class << self
      def for(path)
        path.is_a?(::FSSM::Support::Pathname) ? path : new("#{path}")
      end
    end
  ...
  class Pathname
    def self.[](pattern)
      Dir[pattern].map! {|d| FSSM::Support::Pathname.new(d) }
    end

    def self.pwd
      FSSM::Support::Pathname.new(Dir.pwd)
    end

    def entries
      Dir.entries(self).map! {|e| FSSM::Support::Pathname.new(e) }
    end
    ...
    def self.glob(pattern, flags = 0)
      ...
      dirs.map! {|path| FSSM::Support::Pathname.new(path) }
  ...
  class Pathname
    def self.join(*parts)
      last_part = FSSM::Support::Pathname.new(parts.last)
      return last_part if last_part.absolute?
      FSSM::Support::Pathname.new(File.join(*parts.reject {|p| p.empty? }))
    end
    ...
    def split
      File.split(self).map {|part| FSSM::Support::Pathname.new(part) }
    end
  ...
  class Pathname
    def find
      Find.find(self) {|path| yield FSSM::Support::Pathname.new(path) }
    end
  end
